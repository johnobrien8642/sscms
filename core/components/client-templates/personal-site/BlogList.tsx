...
const BlogList = ({}) => {
	...
	useEffect(() => {
		...
		async function getTotalBlogPosts() {
			const res = await fetch(`/api/get_total_blog_posts/?search=${search}`);
	...
	useEffect(() => {
		...
		async function getBlogPosts() {
			if (blogPostsCount) {
				const res = await fetch(`/api/get_blog_posts/?search=${search}&skip=${currentPage === 1 ? 0 : (currentPage * perPage) - perPage}`);
				const resData = await res.json();
				const { posts } = resData;
				setBlogPosts(posts)
			} else {
